...
export const MouseHoverCheck: React.FC<MouseTrackerProps> = ({ data }) => {
  const [hoveredElementId, setHoveredElementId] = useState<string | null>(null);
  const [lastInsideElementId, setLastInsideElementId] = useState<string | null>(null);

  const setInsideBlockAndLog = (elementId: string | null) => {
    const inside = elementId !== null ? 'Yes' : 'No';
    if (elementId !== lastInsideElementId) {
      if (elementId) {
        console.log(`Inside the block: ${inside}, Element ID: ${elementId}`, data);
      } else {
  ...
  useEffect(() => {
    const ids = ['fullname', 'date', 'number', 'otp', 'animal', 'switch1', 'switch2', 'button'];
    ...
    if (element) {
      const handleChange = (event: Event) => {
        ...
        if (hoveredElementId !== id && hasChanged) {
          console.log(`Suspicious activity detected: Input changed on ${id} but mouse not over it.`);
        } else if (hasChanged) {
  ...
  const addDropdownChangeHandler = () => {
    const dropdown = document.getElementById("animal") as HTMLSelectElement | null;
    if (dropdown) {
      ...
      const handler = () => {
        if (!isMouseOver) {
          console.log("Suspicious activity detected: Dropdown changed on animal but mouse not over it.");
        } else {
  ...
  const addDateChangeHandler = () => {
    ...
    if (dateInput) {
      ...
      const handleChange = () => {
        if (!isMouseOver) {
          console.log("Suspicious activity detected: Date changed but mouse not over it.");
        } else {
    ...
    if (element) {
      const handler = () => {
        if (hoveredElementId !== id) {
          console.log(`Suspicious activity detected: Switch toggled on ${id} but mouse not over it.`);
        } else {
    ...
    if (button) {
      const handler = () => {
        if (hoveredElementId !== "button") {
          console.log(`Suspicious activity detected: Button clicked on button but mouse not over it.`);
        } else {
